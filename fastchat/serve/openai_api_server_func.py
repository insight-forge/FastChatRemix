...
def _add_to_set(s, new_stop):
    ...
    if isinstance(s, str):
        ...
        new_stop.update(s)
def get_function_system(model_name, functions):
    if "qwen" in model_name.lower():
        function_templete = '''You have access to call the following functions with appropriate input when necessary:\n{functions}'''
        return function_templete.format(functions=json.dumps(functions), ensure_ascii=False)
    else:
        return None
...
async def create_chat_completion(request: ChatCompletionRequest):
    ...
    if request.stream and not request.functions:
