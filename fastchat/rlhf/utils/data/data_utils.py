...
def get_raw_dataset_split_index(local_rank, output_path, dataset_name, seed,
                                ...
                                data_size):
    # 将数据按预定比例切分为三份，分别作为RLHF的三个阶段的训练数据并分别打乱，切分后的index缓存在文件中
...
def create_dataset_split(current_dataset, raw_dataset, train_phase, tokenizer,
    ...
    if train_phase == 1:
        ...
        # {"input_ids": [3，2，1], "attention_mask":[1,1,1]} 对prompt tokenizer()后进行了反序
...
def create_dataset(local_rank, dataset_name, data_split, output_path,
    ...
    # 数据切分(eg. data_split=2,4,4)，分别用于RLHF的三个阶段
