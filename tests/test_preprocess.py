...
def test_preprocess():
    ...
    with open(path, 'r', encoding='utf8') as f:
        raw_data = json.load(f)[2:4]
        # raw_data[0]["conversations"] = raw_data[0]["conversations"] + raw_data[1]["conversations"]
        # raw_data = raw_data[:1]
        print(len(raw_data[0]["conversations"]))

    tokenizer = AutoTokenizer.from_pretrained("Qwen/Qwen-7B", trust_remote_code=True, model_max_length=512)
